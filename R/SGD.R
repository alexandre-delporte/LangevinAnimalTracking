...
SGD_Fisher <- function(data, sde_params, fixpar = NULL, SGD_iter,
  ...
  for (k in 1:SGD_iter) {
    ...
    if (verbose) {
      ...
      message(cat("v_k_true:", round(colSums(true_grad_mat), 4),"\n"))
      message("I_k:", round(I_k, 4), "\n")
      message("eigenvalues I_k:", round(eigen(I_k)$values, 4), "\n")
      message("direction:", paste(round(direction, 4), collapse = " "), "\n")
      message("theta:", paste(round(theta[k + 1, ], 4),collapse=" "), "\n")
