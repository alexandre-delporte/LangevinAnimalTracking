...
#' @param scheme numerical scheme for SDE simulation ("Lie-Trotter" or "Strang")
#' @param prior_tau log_normal prior parameters for tau (list with mean and sd on log scale)
#' @param prior_nu  log_normal prior parameters for nu (list with mean and sd on log scale)
#' @param prior_omega normal prior parameters for omega (list with mean and sd)
#' @param fixpar character vector with names of parameters to keep fixed during the inference
#' @param initial_cov initial proposal covariance matrix for the transformed parameters
...
#' -theta_transformed: matrix of transformed parameter values at each iteration
...
#' - final_proposal_cov: final proposal covariance matrix used
#'
#' @export
...
PMMH_langevin <- function(data, theta_init, num_iterations, num_particles,
                          ...
                          polygon, U0, lambda,scheme="Lie-Trotter",
  ...
  # Transform tau and nu in log-scale, keep omega in original scale
  ...
  # Running sum for covariance computation
  sum_theta <- theta_trans
  sum_theta_sq <- outer(theta_trans, theta_trans) #theta*theta^T
  ...
  for (iter in 2:num_iterations) {
    ...
    if (iter > t0) {
      ...
      # Empirical covariance: E[theta*theta^T] - E[theta]E[theta]^T
    ...
    if (is.null(L)) {
      ...
      if (verbose) cat("  Warning: Proposal covariance not positive definite, using initial cov\n")
      L <- initial_cov
    }
    
    # Propose new parameter 
    theta_trans_prop <- theta_trans_current + as.vector(t(L) %*% rnorm(p_free))
  ...
  # Post-processing
  burnin <- max(t0, round(num_iterations * 0.2))  # burnin = t0 or 20 percent of num_iterations
